...
@section('content')
    ...
    <div class="max-w-3xl mx-auto mt-10 p-6 bg-white rounded-xl shadow-md">
        ...
        <form action="{{ route('type_books.update', $type_book->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
@method('PUT')
            {{-- Name --}}
            <div class="input-area">
                ...
                <div>
                    <input id="name" name="name" type="text" value="{{ old('name', $type_book->name) }}"
          ...
          {{-- Image --}}
            <div class="multiFilePreview">
                ...
                @if ($type_book->image)
                    <div class="mt-2">
                        <img src="{{ asset('storage/' . $type_book->image) }}" alt="Current Image" class="h-24 rounded">
                    </div>
                @endif
